...
public:
    ...
    InGameOverlay(unsigned int powerCount = 0, unsigned int simCoinCount = 0, unsigned int temperatureC = 0, unsigned int sunPower = 0, Vector* windSpeed = new Vector {0, 0, 0}, unsigned int timeLeft = 0) {
        activeHud = true;
    ...
    void loadHUDTexture(unsigned int powerCount, unsigned int simCoinCount, unsigned int temperatureC, unsigned int sunPower, unsigned int windSpeed, unsigned int timeLeft){
        ...
        models["skipturn"] = new Button ("skipturn", 0.0, 0.0, 0.1, 175.0, 60,EntityManager::get<Texture2d*>("skipTurn")->ID);
        models["skipturn"]->onClick = [this]() {};

        models["structure"] = new Button ("structure", 0.0, 630.0, 0.1, 90.0, 90, EntityManager::get<Texture2d*>("structure")->ID);
        models["structure"]->onClick = [this]() { activeStructureSideWindow(); };

        models["machine"] = new Button ("machine", 90.0, 630.0, 0.1, 90.0, 90,EntityManager::get<Texture2d*>("machine")->ID);
        models["machine"]->onClick = [this]() { activeMachineSideWindow(); };

        models["cablage"] = new Button ("cablage", 180.0, 630.0, 0.1, 90.0, 90,EntityManager::get<Texture2d*>("wire")->ID);
        models["cablage"]->onClick = [this]() { activeWireSideWindow(); };

        models["info"] = new Button ("info", 270.0, 630.0, 0.1, 90.0, 90,EntityManager::get<Texture2d*>("info")->ID);
        models["info"]->onClick = [this]() { activeInfoSideWindow(); };

        models["delete"] = new Button ("delete", 360.0, 630.0, 0.1, 90.0, 90, EntityManager::get<Texture2d*>("delete")->ID);
        models["delete"]->onClick = [this]() { activeDeleteSideWindow(); };

        //Image2D
        logoList.push_back(new Image("", 175.0, 0.0, 0.1, 540.0, 60, EntityManager::get<Texture2d*>("topBar")->ID));
    ...
    void updateAlerts(std::list<char> listeAlertes){
        ...
        for (auto it : listeAlertes) {
            ...
            if(alertCount < 7) //Affiche les alertes dans la colonne de gauche
                switch (it){
                    case 'p'://pluie/acide
                        alertsList.push_back(new Image("", 0.0 ,150.0 + (alertCount * 60.0), 0.1, 60.0, 60.0, EntityManager::get<Texture2d*>("alert")->ID));
                        break;
                ...
                switch (it){
                    case 'p'://pluie/acide
                        alertsList.push_back(new Image("", 60.0 ,150.0 + ((alertCount % 7) * 60.0), 0.1, 60.0, 60.0, EntityManager::get<Texture2d*>("alert")->ID));
                        break;
