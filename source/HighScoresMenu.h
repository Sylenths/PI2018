...
private:
    Scores* scores[11];
    Font* font = EntityManager::get<Font*>("font - arial28");
...
public:
    ...
    HighScoresMenu(){
        ...
        models["backButtonHighscore"]->onClick = [this]() { Scene::popScene(); };
        ...
        for (int j = 0; j < 10 ; ++j) {
            ...
            labelModels[labelNameBuffer] = new Label("", EntityManager::get<Font*>("font - arial28")->getFont(), {128,128,128,0},labelbuffer, x, y, 0.1);
            y += 75;
...
/// Permet de prendre les scores du fichier et les mettres dans le tableau.
    ...
    void updateScore(std::string name, unsigned int score){
       ...
       scores[10] = scoreBuffer;
        ...
        labelNameBuffer = std::string("HighscoreLabel") + intCharBuffer;
        ((Label*)labelModels[labelNameBuffer])->updateTextTexture(labelbuffer,EntityManager::get<Font*>("font - arial28")->getFont(),{128,128,128,0});
    }
