...
public:
    ...
    TextWindow(const std::string name, TTF_Font* font, SDL_Color color, std::string text, unsigned int x, unsigned int y, unsigned int z, unsigned int w, unsigned h, Model* modelToSubscribe = nullptr, unsigned int textureID = 0) : Model(name, x, y, z, textureID, false) {
        this->text = text;
